import pandas as pd
from scipy import corrcoef,array
import os,re
...
def compute_sentiment_text(text):
    sentiment = 0
    try:
        sentiment = sum([sum([w.polarity for w in s.words]) for s in text.sentences])
    except:
        pass
    return sentiment 
...
def compute_sentiment(fn=EUFN,download=False):
    ...
    df['Sentiment'] = df.TextPoly.apply(compute_sentiment_text)
    sentiment = {k:(df[df.pol_group==k].Sentiment.mean(),v) for k,v in SEATS.item   
