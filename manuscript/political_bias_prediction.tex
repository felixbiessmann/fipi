...
Ideally one would choose for each topic a sample of reports from the entire political spectrum in order to form an unbiased opinion. But ordering media content with respect to the political spectrum at scale requires automated prediction of political bias. The aim of this study is to provide some empirical evidence indicating that leveraging open data sources automated political bias prediction is possible with above chance accuracy. 

When human experts determine political bias of texts they will take responsibility for what they say about a text, and they can explain their decisions. This is a key difference to many statistical learning approaches. Not only is the responsibility question problematic, it can also be difficult to interpret some of the decisions. In order to validate the predictions of the models three strategies that allow for better interpretations of the models are proposed. First interpretations of the model misclassifications are related to changes in party policies. Second univariate measures of correlation between text features and party affiliation allow to relate the predictions to the kind of information that political experts use for interpreting texts. Third sentiment analysis is used to investigate whether this aspect of language has discriminatory power. 
...
A similar approach to the one presented here was taken in \cite{Yu2008}. The authors extracted BOW feature vectors and applied linear classifiers to predict political party affiliation of US congress speeches. They used data from the two chambers of the US congress, House and Senat, in order to investigate generalization performance of a classifier trained on one data source and tested on another. They found that generalization of the classifier as quantified by cross-validation was in some cases strongly affected by the domain. Generalization was also affected by the time difference between the political speeches used for training and those used for testing. 
%
Other work has focused on developing dedicated methods for predicting political bias, what is often referred to as {\em scaling} in these studies; two have become particularly popular, WordFish \cite{Slapin08ascaling} and WordScores \cite{Laver2003}. Both methods have been applied in many studies and their strengths and weaknesses compared, which led to several improvements, see e.g. \cite{Lowe09scalingpolicy}. Validation is done by comparing parameter estimates of the different models amongst each other or with human expert annotations. Models are usually not evaluated by testing them on held-out data, which renders comparisons with this work or other work such as \cite{Yu2008} difficult. In general these approaches have been very valuable for a posteriori analysis of historical data but do not seem to be used as much for analyses of new data in a predictive analytics setting. 
%
Also texts from social network data were investigated in the context of automated political text analysis in the 2008 U.S. presidential election \cite{Gryc2014}. The study highlighted the importance of features such as sentiment or social network structure but the results also show that standard BOW features combined with linear classifiers yield competitive results for sentiment classification on political tweets. 
%
Finally Hirst et al conducted a large number of experiments that offer an interesting comparison to the present study both in terms of methodology but also with respect to their results \cite{Hirst2014}. The authors show that a classifier trained on parliament speeches uses language elements of defense and attack to classify speeches, rather than ideological vocabulary. They also argue that emotional content plays an important role in automatic analysis of political texts. Furthermore their results show a clear depencency between length of a political text and the accuracy with which it can be classified correctly. 

Taken together, there is a large body of literature on an ever expanding field in which scientists from quantitative empirical disciplines as well as political science experts collaborate on the challenging topic of automated analysis of political texts. However except for few exceptions most previous work has focused to a large extent on binary classification\footnote{Many parliaments only have two parties and many studies chose binary classification schemes within manually defined topics and generic schemes such as conservative vs liberal or left vs right.} or to assignment of a one dimensional policy position (mostly left vs right). Often this is a too coarse approach to take into account subtle differences in political policies. This work focuses on more fine grained political view prediction: for one the case of the german parliament is more diverse than two parliament systems, allowing for a distinction between more policies, and second the political view labels considered are more fine grained than in previous studies. Another important contribution of this work is that many existing studies are primarily concerned with a posteriori analysis of historical data. This work aims at prediction of political bias on out-of-domain data with a focus on the practical application on new data, for which a prototypical web application is provided. The experiments follow the established principles of cross validation to estimate out-of-domain generalization and complement the work of \cite{Yu2008} and \cite{Hirst2014} by evaluating data from the german parliament. We confirm and extend some of their findings on why and when generalization performance is impacted negatively. 
...
This study presents a simple approach for automated political bias prediction. The results of these experiments show that automated political bias prediction is possible with above chance accuracy in some cases. 
...
For the cases where evaluation data from two domains was available there was a pronounced drop in performance between in-domain evaluation set and out-of-domain evaluation set. This effect was reported previously, see e.g. \cite{Yu2008}. Also the finding that shorter texts are more difficult to classify than longer texts is in line with previous studies \cite{Hirst2014}. However when considering texts of sufficient length (for instance by aggregating all texts of a given political topic, see \autoref{tab:results_topic}) the classification performance became better and in some cases reliable predictions can be obtained even beyond the training text domain.
