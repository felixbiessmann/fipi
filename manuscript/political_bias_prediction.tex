...
A similar approach to the one presented here was taken in \cite{Yu2008}. The authors extracted BOW feature vectors and applied linear classifiers to predict political party affiliation of US congress speeches. They used data from the two chambers of the US congress, House and Senat, in order to investigate generalization performance of a classifier trained on one domain and tested on another. They found that accuracies of the model when trained on one domain and test on another was significantly decreased. Generalization was also affected by the time difference between the political speeches used for training and those used for testing. 

Other work has focused on developing dedicated methods for predicting political bias such as for instance two methods known as WordFish \cite{Slapin08ascaling} and WordScores \cite{Laver2003}, respectively, and improved versions thereof, see e.g. \cite{Lowe09scalingpolicy}. Validation in these studies is often done by comparing parameter estimates of the different models amongst each other or with human expert annotations. These models are often not evaluated on held-out data or with respect to the same type of labels used to train or partition the data, which renders comparisons with this work or other work such as \cite{Yu2008} difficult. These approaches have been very valuable for a posteriori analysis of historical data but do not seem to be used as much for analyses of new data in a predictive analytics setting. 
%
...
This study presents a simple approach for automated political bias prediction. The results of these experiments show that automated political bias prediction is possible with above chance accuracy in some cases. It is worth noting that even if the accuracies are (as those of comparable studies \cite{Yu2008, Hirst2014}) are not perfect, they nonetheless are on average above chance. While these results do not allow for usage in production systems for classification, it is well possible to use such a system as assistive technology for human annotators in an active learning setting.
...
For the cases where evaluation data from two domains was available there was a pronounced drop in performance between in domain evaluation set and out of domain evaluation set. This effect was reported previously for similar data, see e.g. \cite{Yu2008}. Also the finding that shorter texts are more difficult to classify than longer texts is in line with previous studies \cite{Hirst2014}. However when considering texts of sufficient length (for instance by aggregating all texts of a given political topic, see \autoref{tab:results_topic}) the classification performance became better; in some cases reliable predictions can be obtained even beyond the training text domain.
